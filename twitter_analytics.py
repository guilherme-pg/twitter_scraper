...
def process_analytics(df):
    ...
    tweets_by_day_aggregated = tweets_by_day.groupby('n_tweet', as_index=False).value_counts()
    ...
    plot.set(title='Tweets per Day')
    plt.text(0.5, 0.92, 'Median as red columns and Mean as the line', fontsize=12,
    ...
    plt.xlabel("Total Tweets per Day")
    # TO IMPROVE: aggregate by post quantity
    # TO IMPROVE: add by the amount of likes
    # TO IMPROVE: aggregate by number of retweets
    # TO IMPROVE: aggregate by number of comments

    plt.savefig("saved_charts/twitter_analytic_mean_median_mode.jpg", dpi=300)
...
'''
    ...
    plt.savefig("saved_charts/twitter_analytic_tweets_by_day.jpg", dpi=300)
'''
'''
    # ~~~~~~~~~~~~~~~~~~  ENGAGEMENT BY QUOTE  ~~~~~~~~~~~~~~~~~~
...
plt.savefig("saved_charts/twitter_analytic_likes_adversaries_citations.jpg",
            dpi=300)
'''
